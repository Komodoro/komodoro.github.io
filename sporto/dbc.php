...
define ("DB_NAME","sporto2"); // set database name
...
/* Specify user levels */
define ("SUPER_LEVEL", 6);
define ("ADMIN_LEVEL", 5);
define ("EVENT_MANAGERS_LEVEL", 3);
define ("USER_LEVEL", 2);
...
/*************** reCAPTCHA KEYS****************/
$publickey = "6LcWXesSAAAAABjjcIrA9biTW_gxAfkyFyoiFGpk";
$privatekey = "6LcWXesSAAAAANogXq0M7NwT0jfVSFqnsyKEeTKW";
...
if (!isset($_SESSION['id']) && !isset($_SESSION['username']) ) 
{
	...
	$rs_ctime = mysql_query("select `ckey`,`ctime` from `users` where `id` ='$cookie_id'") or die(mysql_error());
...
/* We also do authentication check of the `ckey` stored in cookie matches that stored in database during login*/
	 ...
	 if( !empty($ckey) && is_numeric($_COOKIE['id']) && isUserID($_COOKIE['username']) && $_COOKIE['user_key'] == sha1($ckey)  ) {
		...
		/* query user level from database instead of storing in cookies */	
		  list($group_id) = mysql_fetch_row(mysql_query("select group_id from users where id='$_SESSION[id]'"));
...
$sess_id = strip_tags(mysql_real_escape_string($_SESSION['id']));
$cook_id = strip_tags(mysql_real_escape_string($_COOKIE['id']));

if(isset($sess_id) || isset($cook_id)) {
mysql_query("update `users` 
			set `ckey`= '', `ctime`= '' 
			where `id`='$sess_id' OR  `id` = '$cook_id'") or die(mysql_error());
}		
...
if($_SESSION['group_id'] == ADMIN_LEVEL) {
return 1;
} else { return 0 ;
}
}
...
if($_SESSION['group_id'] == SUPER_LEVEL) {
return 1;
} else { return 0 ;
}
}
...
if($_SESSION['group_id'] == EVENT_MANAGERS_LEVEL) {
return 1;
} else { return 0 ;
}
}
...
if($_SESSION['group_id'] == GUEST_LEVEL) {
return 1;
} else { return 0 ;
}
}
